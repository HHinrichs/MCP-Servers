...
function tokenize(text: string): string[] {
  return text
    .toLowerCase()
    // Drop markdown noise that confuses ranking.
    .replace(/```[\s\S]*?```/g, " ")     // fenced code blocks
    .replace(/`[^`]*`/g, " ")            // inline code
    .replace(/\[\[([^\]|]+)(\|[^\]]+)?\]\]/g, " $1 ") // wikilinks keep target text
    .replace(/!?\[([^\]]*)\]\([^)]*\)/g, " $1 ")      // md links keep text
    .replace(/^---[\s\S]*?---/m, " ")    // strip YAML frontmatter
    .replace(/[^\p{L}\p{N}]+/gu, " ")    // non-word chars
    .split(/\s+/)
...
function buildSnippet(content: string, queryTokens: Set<string>, contextChars = 100): string {
  // Find the densest stretch of query-token hits; quick & dirty.
  ...
  for (let i = 0; i < tokens.length; i += 2) {
    ...
    if (t && queryTokens.has(t)) {
      // count hits in a 30-token window
      let s = 0;
  ...
  // Render around bestIdx as char window.
  let charPos = 0;
...
export const findSimilarTool = {
  name: "find_similar",
  description:
    "Find existing notes in the vault that are semantically similar to a given text, using TF-IDF ranking. ALWAYS call this before add_to_project / add_to_area / add_to_resource to decide whether to extend an existing note instead of creating a new one. Returns ranked matches with relevance score and a snippet.",
  inputSchema: {
  ...
  handler: async ({ text, limit = 5 }: { text: string; limit?: number }) => {
    ...
    if (queryTokens.length === 0) {
      return {
        content: [{ type: "text" as const, text: "Query enthält nur Stopwörter — keine sinnvolle Ähnlichkeitssuche möglich." }],
      };
    }
    ...
    if (docs.length === 0) {
      return {
        content: [{ type: "text" as const, text: "Vault enthält keine durchsuchbaren Notizen." }],
      };
    }
    ...
    for (const doc of docs) {
      ...
      if (score > 0) {
        hits.push({
          path: doc.path,
          score,
          snippet: buildSnippet(doc.snippetSource, queryTokenSet),
        });
      }
    }
    ...
    if (top.length === 0) {
      return {
        content: [{ type: "text" as const, text: "Keine ähnlichen Notizen gefunden — der Inhalt scheint neu zu sein." }],
      };
    }

    const summary =
      `${top.length} ähnliche Notiz(en) gefunden (Score 0..1, höher = ähnlicher):\n\n` +
      top
        .map(
          (h, i) =>
            `${i + 1}. **${h.path}** _(score ${h.score.toFixed(3)})_\n   ${h.snippet}`,
        )
        .join("\n\n") +
      `\n\n_Wenn ein Treffer ≥ 0.15 thematisch passt, erweitere die bestehende Datei via add_to_project/add_to_area/add_to_resource statt eine neue anzulegen._`;
